...
let allTheNotes =  ["C1", "C#1", "D1", "D#1", "E1", "F1", "F#1", "G1", "G#1", "A1", "A#1", "B1",
...
let major = [0,2,4,5,7,9,11,12,14]; // intervals for a major scale for 9 notes
let pentatonic = [0,2,4,7,9,12,14,16,19]; // intervals for a pentatonic scale for 9 notes
let minor = [0,2,3,5,7,8,10,12,14]; // intervals for a minor scale for 9 notes
let majorBlues = [0,2,3,4,7,9,12,14,15]; // intervals for a major blues scale for 9 notes
let minorBlues = [0,3,5,6,7,10,12,15,17]; // intervals for a minor scale for 9 notes
let scales = ["default", pentatonic, major, minor, majorBlues, minorBlues];
let scale; // variable to store the scale in
let theKey = 0; // this variable sets the default key on load
let octave = 36; //set the default octave on load
let synth;
